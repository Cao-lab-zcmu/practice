...
awk -F $'\t' -v OFS=$'\t' '{if(NR>=2){print $3,$14}}' results/finfirst_set.tsv > temp/Mo_filename;\
	...
	sourceion=$(grep ionMass *_$source/compound.info | awk '{print $2}' | sed 's/[[:space:]]//g')
	target=$(awk '{if(NR=='$i'){print $2}}' temp/ftaligntemp/filter_net_$tlimit)
	targetion=$(grep ionMass *_$target/compound.info | awk '{print $2}' | sed 's/[[:space:]]//g')
	deltaion=$(echo "$sourceion - $targetion"|bc)
